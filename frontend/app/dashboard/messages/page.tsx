...
function mapChatStateLabel(state: ChatThreadSummary["chat_state"]) {
  ...
  if (state === "pending_acceptance") return "Awaiting Your Acceptance";
  return "Awaiting HR Request";
}
...
function mapMessages(items: ChatMessageRecord[]): UiMessage[] {
  return items.map((item) => ({
    id: item.id,
    sender: item.sender_type === "user" ? "you" : "hr",
...
export default function MessagesPage() {
  ...
  }, [messages.length, hrTyping]);

  useEffect(() => {
    if (!selectedComplaintCode || !threadDetails || threadDetails.chat_state !== "active") return;

    const lastHrMessage = [...messages].reverse().find((item) => item.sender === "hr");
    if (!lastHrMessage) return;

    if (seenState.user_last_seen_message_id === lastHrMessage.id) return;
    ...
    socket.emit("chat:seen", {
      ...
      messageId: lastHrMessage.id,
    });
  }, [messages, selectedComplaintCode, seenState.user_last_seen_message_id, threadDetails]);
  ...
  const hrSeenCutoff = useMemo(() => {
    return deriveSeenCutoff(messages, seenState.hr_last_seen_message_id);
  }, [messages, seenState.hr_last_seen_message_id]);
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
      await emitWithAck("chat:accept_request", {
        complaintCode: selectedThread.complaint_code,
      });
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Unable to accept chat request.");
    } finally {
          ...
          <div className="flex w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <aside className="w-full max-w-sm border-r border-slate-200 bg-slate-50/60">
              <div className="border-b border-slate-200 p-4">
                ...
                <p className="mt-1 text-xs text-slate-600">Anonymous chat with HR by complaint code only</p>
              ...
              <div className="h-[calc(100%-114px)] overflow-y-auto p-2">
                {threadsLoading ? <p className="px-2 py-3 text-sm text-slate-600">Loading threads...</p> : null}
                {threadsError ? (
                ...
                {visibleThreads.map((thread) => (
                  ...
                  >
                    ...
                    <p className="mt-1 truncate text-xs text-slate-600">{thread.last_message_preview || mapChatStateLabel(thread.chat_state)}</p>
                ...
                <>
                  <header className="border-b border-slate-200 bg-white p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">Complaint: {selectedThread.complaint_code}</p>
                        <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-600">
                          <StatusBadge status={selectedThread.complaint_status} />
                          <span className="rounded-full border border-slate-300 bg-slate-50 px-2 py-1">{mapChatStateLabel(selectedThread.chat_state)}</span>
                    ...
                    {selectedThread.chat_state === "pending_acceptance" ? (
                      <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                        <p className="font-semibold">HR has requested to start secure chat for this complaint.</p>
                    ...
                    {selectedThread.chat_state === "not_requested" ? (
                      <p className="mt-3 rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-xs text-slate-700">
                        Chat will activate once HR sends an initiation request for this complaint.
                      </p>
                    ) : null}
                  </header>
                  ...
                  <div className="flex-1 overflow-y-auto bg-slate-50/40 px-4 py-4">
                          ...
                          {group.messages.map((message) => (
                            <MessageBubble
                              ...
                              isSeen={
                                message.sender === "you" &&
                                hrSeenCutoff !== null &&
                                new Date(message.timestamp).getTime() <= hrSeenCutoff
                              }
                            />
                          ))}
                        </div>
                    ...
                    {hrTyping ? (
                      <p className="text-xs text-slate-600">HR is typing...</p>
                    ) : null}
                  ...
                  <footer className="border-t border-slate-200 bg-white p-3">
                    {actionError ? (
                      <p className="mb-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{actionError}</p>
                    ) : null}
                    ...
                    {!canSend ? (
                      <p className="rounded-lg border border-slate-300 bg-slate-100 px-3 py-2 text-sm text-slate-600">
                        {isClosedStatus(selectedThread.complaint_status)
                          ...
                          : "Messaging will be enabled after request acceptance."}
                      </p>
                    ) : (
...
function MessageBubble({ message, isSeen }: { message: UiMessage; isSeen: boolean }) {
  ...
  return (
    <div className={`flex ${isYou ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[78%] rounded-xl px-3 py-2 text-sm ${isYou ? "bg-slate-900 text-white" : "border border-slate-300 bg-white text-slate-800"}`}>
...
function StatusBadge({ status }: { status: ChatThreadSummary["complaint_status"] }) {
  ...
  return <span className={`inline-flex rounded-full border px-2 py-1 text-[11px] font-semibold ${styles[status]}`}>{mapStatusLabel(status)}</span>;
}
